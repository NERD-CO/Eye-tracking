function [groupStats] = eyeTrack_StatPrep_allSubjects_v3(cleanedDataLOC, conditiON, subStep)


%%%% STEP 1
% Get rid of the ptID input argument 
pt_ID = ["AMC_PY21NO05", "AMC_PY22NO09", "AMC_PY22NO12", "AMC_PY22NO13", "AMC_PY22NO16"];
cd(cleanedDataLOC)
allPatients = struct; % create empty struct
for allPTs = 1:length(pt_ID)
    ...
    allPatients.(pt_ID{allPTs}) = variantS;
end 
...
for pi = 1:length(patientFieldNames)
    ...
    for vi = 1:length(tempVariants)
        allVariants = [allVariants; tempPatient.(tempVariants{vi}).dataTable];
    end
end
...
% 
%             allVariants = variantS.(variNUM{vi}).dataTable;

%             if matches(varPtInfoLearn.Eye2Use{1},'Left')
                learnDATA = allVariants.Left_L_oT_pupilS_rawCL;
%             else
%                 learnDATA = allVariants.Right_L_oT_pupilS_rawCL;
%             end

            learnRespN = allVariants.LearnResp;
            ...
            learnGT = allVariants.Recog_CatID;

            learnTTL = allVariants.LearnTTLplus;
...
%             variNum = variNUM{vi};
            ...
            [statsOutput] = getStatsTable(inData , learnTTL ,...
                trialGroupIDs, trialGroupInds);
            groupStats= statsOutput;
...
%             if matches(varPtInfoRecog.Eye2Use{1},'Left')
                 RecogDATA = allVariants.Left_R_oT_pupilS_rawCL;
%             else
%                 RecogDATA = allVariants.Right_R_oT_pupilS_rawCL;
%             end




        RecogTTL = allVariants.RecogTTLplus;
        RecogRespN = allVariants.RecogResp;
        RecogResp = RecogRespN;
        RecogGT = allVariants.groundTruth;
        ...
        switch subStep
            case 1
                ...
                [statsOutput] = getStatsTable(inData , RecogTTL ,...
                    trialGroupIDs, trialGroupInds);
                groupStats = statsOutput; 
...
switch conditiON
    ...
    case 'learn'
        ...
        %for vi = 1:length(variNUM)
            ...
            ydata = groupStats.StatTable.PupilSize;
            condition2u = groupStats.StatTable.GroupID;
            epoch2u = groupStats.StatTable.EpochID;
            ...
            groupStats.Condition.Posthoc = tbl_condition;
            groupStats.Condition.Anova = anovaSTAT_condition;

            groupStats.Epoch.Posthoc = tbl_epoch;
            groupStats.Epoch.Anova = anovaSTAT_epoch;
...
function [statsOutput] = getStatsTable(pupilData , ttlDATA ,...
