function [] = EyeAnalysis_DataExtract_v2(excelLOC , mainLOC, ptID, saveLOC)
...
for vi = 1:length(allvars)
    ...
    [leftEYE_Learn , rightEYE_Learn , learnTTLinfo] = getEYErawEpoch(rawT_L , tsT_L , trialID_L);
    ...
    [leftEYE_Recog , rightEYE_Recog , recogTTLinfo] = getEYErawEpoch(rawT_R , tsT_R , trialID_R);
...
%%%%% NEW RAW will go from -200ms [from start] to +200ms [from end]
...
for tttrialir = 1:length(trialsOfInt)
    ...
    % 500ms after last TS
    endTS   = tmpOtrTAB.timeStamp(tmpOtrTAB.TTLid == 6) + 250;

    tmpTable = table;
    tmpTable.TrialNUM = repmat(tmpOtr,height(tmpOtrTAB) + 2,1);
    tmpTable.EventID = [nan ; tmpOtrTAB.TTLid ; nan];
    tmpTable.EventDesc = {'m200ms' ; 'StimOn'; 'StimOff';'QuestionScr';...
