function [] = Initial_EyeAnalysis_MLD_v6(excelLOC , mainLOC, ptID, basePath, saveLOC)
...
for vi = 1:length(allvars)
    ...
    [eyeProcL] = Edf2Mat_UCH(basePath, learnEDF{1}, outFOLDS{ptIdx}, 'Learn',...
        varianT, tempCASEd, tempCASEd);
    [eyeProcR] = Edf2Mat_UCH(basePath, recogEDF{1}, outFOLDS{ptIdx}, 'Recog',...
        varianT, tempCASEd, tempCASEd);
    ...
    cd(mainLOC); % Used to create variable 'outData' with TTL values
    ...
    %           learnTXT = vartiTAB.Behavior(matches(vartiTAB.Block,'learn'));
    recogTXT = vartiTAB.Behavior(matches(vartiTAB.Block,'recog'));

    %                 % Learn file
    %                     textloc = string(learnTXT);
    %                     block = 'learn'; %Will be either 'learn' or 'recog'
    %                     patientID = idTab.Subject{1};
    %
    %                   % Run function and load in new .mat file %%%DONT NEED RIGHT NOW
    %                   [behavFILE_learn] = NewOldTxttoMat_v2(textloc,patientID,vi,block, tempCASEd);
    %                   load(behavFILE_learn, 'outData') %loc and file name as input

    % Recog file
    textloc = string(recogTXT);
    block = 'recog'; %Will be either 'learn' or 'recog'
    patientID = idTab.Subject{1};

    % Run function and load in new .mat file
    [behavFILE_recog] = NewOldTxttoMat_v2(textloc,patientID,vi,block, tempCASEd);
